...
use windows::{
    ...
    Win32::{
        ...
        System::{
            Console::{
                GetConsoleMode,
                GetConsoleScreenBufferInfo, SetConsoleMode, CONSOLE_MODE,
...
impl Drop for Process {
    fn drop(&mut self) {
        unsafe {
            if dynamic_api::is_conpty_available() {
                let _ = unsafe { dynamic_api::close_pseudo_console(self._console) };
            } else {
                // On systems without ConPTY support, skip the call
            }
...
fn createPseudoConsole(size: COORD) -> win::Result<(HPCON, HANDLE, HANDLE)> {
    ...
    // Check if ConPTY APIs are available before attempting to use them
    if !dynamic_api::is_conpty_available() {
        return Err(win::Error::new(
            HRESULT::from_win32(0),
            "ConPTY APIs are not available on this system. Windows 10 or later is required.",
        ));
    }

    let console = unsafe {
        match dynamic_api::create_pseudo_console(size, pty_in, pty_out, 0) {
            Ok(h) => h,
            Err(_) => return Err(win::Error::new(
                HRESULT::from_win32(0),
                "Failed to create pseudo console - API not available", 
            )),
        }
    };
...
fn resize_console(console: HPCON, x: i16, y: i16) -> Result<(), Error> {
    if dynamic_api::is_conpty_available() {
        unsafe {
            match dynamic_api::resize_pseudo_console(console, COORD { X: x, Y: y }) {
                Ok(_) => Ok(()),
                Err(_) => Err(Error::Win(windows::core::Error::new(
                    HRESULT::from_win32(0),
                    "ResizePseudoConsole API not available",
                ))),
            }
        }
    } else {
        Err(Error::Win(windows::core::Error::new(
            HRESULT::from_win32(0),
            "ConPTY APIs are not available on this system. Windows 10 or later is required.",
        )))
    }
